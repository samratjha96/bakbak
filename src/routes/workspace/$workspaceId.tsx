...
import { workspaceDetailsQuery, workspaceRecordingsQuery } from '~/lib/workspaceQueries'
...
export const Route = createFileRoute('/workspace/$workspaceId')({
  loader: async ({ params: { workspaceId }, context }) => {
    ...
    try {
      ...
      throw new Response(error instanceof Error ? error.message : 'Workspace not found', {
...
function WorkspacePage() {
  ...
  const { data: recordings = [] } = useQuery(workspaceRecordingsQuery(workspaceId))
      ...
      <div>
        ...
        {recordings.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-lg">
            ...
            <p className="text-gray-600 mb-4">Get started by creating your first recording in this workspace.</p>
            <Link
              to="/recordings/new"
              search={{ workspaceId }}
              className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"
            ...
            {recordings.map((recording) => (
              ...
              >
                ...
                {recording.description && (
                  <p className="text-sm text-gray-600 mb-2 line-clamp-2">{recording.description}</p>
                )}
                
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <div className="flex items-center gap-2">
                    {recording.language && (
                      <span className="px-2 py-1 bg-gray-100 rounded">{recording.language}</span>
                    )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-6">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
              <option value="viewer">Viewer - Can view recordings and add comments</option>
              <option value="editor">Editor - Can create and edit recordings</option>
              {canInviteOwners && (
