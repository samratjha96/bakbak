...
const RecordingsSidebar: React.FC<{activities: {id: string; text: string; time: string}[]}> = ({ activities }) => {
  return (
    ...
    >
      <div className="container py-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          ...
          <StatCard value={recordings.filter(r => {
            const now = new Date();
            const weekAgo = new Date(now.setDate(now.getDate() - 7));
            return new Date(r.createdAt) > weekAgo;
          }).length} label="This Week" />
          <StatCard value={`${Math.round(recordings.reduce((acc, r) => acc + r.duration, 0) / 60 / 60 * 10) / 10}h`} label="Practice Time" />
          <StatCard value={new Set(recordings.map(r => r.language).filter(Boolean)).size} label="Languages" />
        </div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
            {recordings.map((recording, index) => (
              <React.Fragment key={recording.id}>
                {index > 0 && <div className="mx-4 border-t border-gray-100 dark:border-gray-800"></div>}
                <RecordingItem
                  id={recording.id}
